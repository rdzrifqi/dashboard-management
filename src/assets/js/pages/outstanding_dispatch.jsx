...
const OutstandingDisptachTable = () => {
    const tableRef = useRef();
    const [outstandingDispatchData, setoutstandingDispatchData] = useState([]);
    ...
    const columns = [
        ...
        { label: "CBM", index: 2, default:true },
        { label: "KGS", index: 3, default:true },
        { label: "Min KGS", index: 4, default:true },
        { label: "Lead Time", index: 5, default:true },
        { label: "Ratio", index: 6, default:true },
        { label: "VM", index: 7, default:true },
        { label: "CW", index: 8, default:true },
        { label: "Cheapest", index: 9, default:true },
        { label: "Vendor Name", index: 10, default:false },
        { label: "Service Name", index: 11, default:false }
    ];
    ...
    const fetchData = (page = 1) => {
        ...
        axios.get(`${__ODOO_URL__}/api/odoo/outstanding-dispatch?search=${searchText}&limit=${limit}&offset=${offset}`)
            .then(res => {
                const result = res.data;

                setoutstandingDispatchData(result.data.data || []);
                        ...
                        <div className="overflow-auto">
                            <table border="1">
                                ...
                                <thead className="text-left" style={{backgroundColor:'#0d2b5e'}}>
                                    <tr>
                                        <th class="text-white w-12 sticky left-0 bg-gray-950 z-4">No</th>
                                        {columns.map(col => 
                                        ...
                                        <th className="text-white sticky right-0 bg-gray-950 z-10">Actions</th>
                                    </tr>
                                </thead>
                                ...
                                <tbody>
                                    {outstandingDispatchData.map((item, index) => (
                                        ...
                                        <td className="sticky left-0 bg-white dark:bg-dark z-4">{index + 1}</td>
                                            ...
                                            {/* Address */}
                                            {visibleColumns.includes(3) && <td>{item.kgs}</td>}

                                            {visibleColumns.includes(4) && <td>{item.min_kgs}</td>}
                                            {visibleColumns.includes(5) && <td>{item.lead_time}</td>}
                                            {visibleColumns.includes(6) && <td>{item.ratio}</td>}
                                            {visibleColumns.includes(7) && <td>{item.vm}</td>}
                                            {visibleColumns.includes(8) && <td>{item.cw}</td>}
                                            {visibleColumns.includes(9) && <td>{item.cheapest}</td>}
                                            {visibleColumns.includes(10) && <td>{item.vendor_name}</td>}
                                            {visibleColumns.includes(11) && <td>{item.service_name}</td>}
...
root.render(<OutstandingDisptachTable />);