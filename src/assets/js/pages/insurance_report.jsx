...
const InsuranceReportTable = () => {
    ...
    const [insuranceReportData, setInsuranceReportData] = useState([]);
    ...
    const columns = [
        ...
        { label: "Invoice No", index: 2, default:true },
        { label: "Invoice Date", index: 3, default:true },
        { label: "Invoice Due Date", index: 4, default:true },
        { label: "Terms", index: 5, default:true },
        { label: "Amount", index: 6, default:true },
        { label: "Payment Date", index: 7, default:true },
        { label: "Payment Days", index: 8, default:true }
    ];
    ...
    const fetchData = (page = 1) => {
        ...
        axios.get(`${__ODOO_URL__}/api/odoo/insurance-report?search=${searchText}&limit=${limit}&offset=${offset}`)
            .then(res => {
                const result = res.data;

                setInsuranceReportData(result.data.data || []);
                                ...
                                <tbody>
                                    {insuranceReportData.map((item, index) => (
                                        <tr key={index} className="align-top">
                                            ...
                                            {visibleColumns.includes(4) && <td>{formatDate(item.invoice_due_date)}</td>}
                                            {visibleColumns.includes(5) && <td>{item.terms}</td>}
...
const root = ReactDOM.createRoot(
    document.getElementById("insurance_report")
);
root.render(<InsuranceReportTable />);