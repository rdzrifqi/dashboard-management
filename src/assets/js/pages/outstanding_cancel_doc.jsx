...
const OutstandingCancelDocTable = () => {
    ...
    const columns = [
        { label: "Cancel Type", index: 1 },
        { label: "Cust PO No", index: 2 },
        { label: "PO Date", index: 3 },
        { label: "Exp Date", index: 4 },
        { label: "Shipment Date", index: 5 },
        { label: "POD Date", index: 6 },
        { label: "Driver", index: 7 },
        { label: "AWB No", index: 8 }
    ];
    ...
    useEffect(() => {
        axios.get(`${__API_URL__}/outstanding_cancel_doc/master`)
            .then(res => setOutstandingCancelDocData(res.data))
            .catch(err => console.error(err));
    }, []); 
    useEffect(() => {
        if (!outstandingCancelDocData || outstandingCancelDocData.length === 0) return;
        ...
        tableInstance.current = $(tableRef.current).DataTable({
            ...
            columns: [
                ...
                { title: "Cancel Type", data: "cancel_type" },
