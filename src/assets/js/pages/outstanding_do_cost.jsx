...
const OutstandingDOCostTable = () => {
    ...
    const columns = [
        ...
        { label: "DC Name", index: 3 },
        { label: "Cust Lead Time (7d)", index: 4 },
        { label: "Freight Lead Time (6d)", index: 5 },
        { label: "Area", index: 6 },
        { label: "Pulau", index: 7 },
        { label: "Provinsi", index: 8 }
    ];
    ...
    useEffect(() => {
        axios.get(`${__API_URL__}/outstanding_do_cost/master`)
            .then(res => setOutstandingDoCostData(res.data))
            .catch(err => console.error(err));
    }, []); 
    useEffect(() => {
        if (!outstandingDoCostData || outstandingDoCostData.length === 0) return;
        ...
        tableInstance.current = $(tableRef.current).DataTable({
            ...
            columns: [
                ...
                { title: "DC Name", data: "dc" },
                { title: "Cust Lead Time (7d)", data: "cust_lead_time" },
                { title: "Freight Lead Time (6d)", data: "freight_lead_time" },
                { title: "Area", data: "area" },
                { title: "Pulau", data: "pulau" },
                { title: "Provinsi", data: "propinsi" }
            ],
        });
