...
const OutstandingGrTable = () => {
    const tableRef = useRef();
    const [outstandingGrData, setOutstandingGrData] = useState([]);
    ...
    const columns = [
        ...
        { label: "Approved Date", index: 4, default:true },
        { label: "Customer", index: 5, default:true },
        { label: "Customer Alias", index: 6, default:true },
        { label: "DC Name", index: 7, default:true },
        { label: "Area", index: 8, default:true },
        { label: "Cust. Lead Time", index: 9, default:false },
        { label: "Product Code", index: 10, default:false },
        { label: "Std Pack", index: 11, default:false },
        { label: "Qty SO", index: 12, default:false },
        { label: "Qty CTN", index: 13, default:false },
        { label: "Repack", index: 14, default:false },
        { label: "Length", index: 15, default:false },
        { label: "Breadth", index: 16, default:false },
        { label: "Height", index: 17, default:false },
        { label: "CM3", index: 18, default:false },
        { label: "CBM", index: 19, default:false },
        { label: "KGS", index: 20, default:false }
    ];
    ...
    const fetchData = (page = 1) => {
        ...
        axios.get(`${__ODOO_URL__}/api/odoo/outstanding-gr?search=${searchText}&limit=${limit}&offset=${offset}`)
            .then(res => {
                const result = res.data;

                setOutstandingGrData(result.data.data || []);
                    ...
                    <div class="col-span-12 2xl:col-span-12 order-[17] card">
                        ...
                        <div class="overflow-auto">
                            <table border="1">
                                ...
                                <tbody>
                                    {outstandingGrData.map((item, index) => (
                                        <tr key={index} className="align-top">
                                            <td className="sticky left-0 bg-white z-4 dark:bg-dark">{(currentPage - 1) * limit + index + 1}</td>

                                            {visibleColumns.includes(1) && <td>{item.cust_po_no}</td>}
                                            {visibleColumns.includes(2) && <td>{formatDate(item.so_date)}</td>}
                                            {visibleColumns.includes(3) && <td>{formatDate(item.exp_date)}</td>}
                                            {visibleColumns.includes(4) && <td>{formatDate(item.approved_date)}</td>}
                                            {visibleColumns.includes(5) && <td>{item.customer}</td>}
                                            {visibleColumns.includes(6) && <td>{item.customer_alias}</td>}
                                            {visibleColumns.includes(7) && <td>{item.dc_name}</td>}
                                            {visibleColumns.includes(8) && <td>{item.area}</td>}
                                            {visibleColumns.includes(9) && <td>{item.cust_lead_time}</td>}
                                            {visibleColumns.includes(10) && <td>{item.product_code}</td>}
                                            {visibleColumns.includes(11) && <td>{item.std_pack}</td>}
                                            {visibleColumns.includes(12) && <td>{item.qty_so}</td>}
                                            {visibleColumns.includes(13) && <td>{item.qty_ctn}</td>}
                                            {visibleColumns.includes(14) && <td>{item.repack}</td>}
                                            {visibleColumns.includes(15) && <td>{item.length}</td>}
                                            {visibleColumns.includes(16) && <td>{item.breadth}</td>}
                                            {visibleColumns.includes(17) && <td>{item.height}</td>}
                                            {visibleColumns.includes(18) && <td>{item.cm3}</td>}
                                            {visibleColumns.includes(19) && <td>{item.cbm}</td>}
                                            {visibleColumns.includes(20) && <td>{item.kgs}</td>}
                                            <td className="sticky right-0 bg-white z-10 dark:bg-gray-900"><i class="ri-eye-line"></i><i class="ri-printer-line ml-3"></i></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
...
root.render(<OutstandingGrTable />);