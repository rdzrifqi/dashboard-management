...
const OutstandingDisptachFreightTable = () => {
    const [outstandingDispatchFreightData, setoutstandingDispatchFreightData] = useState([]);
    ...
    const columns = [
        ...
        { label: "Price", index: 9, default:false },
        { label: "Est. Courier Cost", index: 10, default:false },
        { label: "Cheapest", index: 11, default:false },
        { label: "Vendor Name", index: 12, default:false },
        { label: "Service Name", index: 13, default:false }
    ];
    ...
    const fetchData = (page = 1) => {
        ...
        axios.get(`${__ODOO_URL__}/api/odoo/outstanding-dispatch-with-freight-cost?search=${searchText}&limit=${limit}&offset=${offset}`)
            .then(res => {
                const result = res.data;

                setoutstandingDispatchFreightData(result.data.data || []);
                                ...
                                <tbody>
                                    {outstandingDispatchFreightData.map((item, index) => (
                                        ...
                                        <td className="sticky left-0 bg-white z-10">{index + 1}</td>
                                            ...
                                            {visibleColumns.includes(9) && <td>{item.price}</td>}
                                            {visibleColumns.includes(10) && <td>{item.est_courier_cost}</td>}
...
root.render(<OutstandingDisptachFreightTable />);