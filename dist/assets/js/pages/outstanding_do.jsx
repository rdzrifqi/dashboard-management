...
const OutstandingDOTable = () => {
    const tableRef = useRef();
    const [outstandingDoData, setOutstandingDoData] = useState([]);
    ...
    const columns = [
        ...
        { label: "DC Name", index: 3, default:true },
        { label: "Area", index: 4, default:true },
        { label: "Pulau", index: 5, default:true },
        { label: "Provinsi", index: 6, default:true },
        { label: "Kabupaten", index: 7, default:true },
        { label: "Kecamatan", index: 8, default:true },
        { label: "Kelurahan", index: 9, default:false },
        { label: "Kode Pos", index: 10, default:false },
        { label: "PO Date", index: 11, default:false },
        { label: "Expired Date", index: 12, default:false },
        { label: "GR No", index: 13, default:false },
        { label: "GR Date", index: 14, default:false },
        { label: "Location", index: 15, default:false },
        { label: "DO No", index: 16, default:false },
        { label: "DO Date", index: 17, default:false },
        { label: "Total SKU PO", index: 18, default:false },
        { label: "Total CTN PO", index: 19, default:false },
        { label: "Total SKU GR", index: 20, default:false },
        { label: "Total CTN GR", index: 21, default:false },
        { label: "Total M3 GR", index: 22, default:false },
        { label: "Total KGS GR", index: 23, default:false },
        { label: "Print Label Date", index: 24, default:false }
    ];
    ...
    const fetchData = (page = 1) => {
        ...
        axios.get(`${__ODOO_URL__}/api/odoo/outstanding-do?search=${searchText}&limit=${limit}&offset=${offset}`)
            .then(res => {
                const result = res.data;

                setOutstandingDoData(result.data.data || []);
                                ...
                                <tbody>
                                    {outstandingDoData.map((item, index) => (
                                        ...
                                        <td className="sticky left-0 bg-white z-10">{index + 1}</td>
                                            ...
                                            {visibleColumns.includes(4) && <td>{item.area}</td>}
                                            ...
                                            {visibleColumns.includes(10) && <td>{item.kode_pos}</td>}
                                            {visibleColumns.includes(11) && <td>{item.po_date}</td>}
                                            {visibleColumns.includes(12) && <td>{item.expired_date}</td>}
                                            {visibleColumns.includes(13) && <td>{item.gr_no}</td>}
                                            {visibleColumns.includes(14) && <td>{item.gr_date}</td>}
                                            {visibleColumns.includes(15) && <td>{item.location}</td>}
                                            {visibleColumns.includes(16) && <td>{item.do_no}</td>}
                                            {visibleColumns.includes(17) && <td>{item.do_date}</td>}
                                            {visibleColumns.includes(18) && <td>{item.total_sku_po}</td>}
                                            {visibleColumns.includes(19) && <td>{item.total_ctn_po}</td>}
                                            {visibleColumns.includes(20) && <td>{item.total_sku_gr}</td>}
                                            {visibleColumns.includes(21) && <td>{item.total_ctn_gr}</td>}
...
root.render(<OutstandingDOTable />);