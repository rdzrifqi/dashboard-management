...
const OutstandingSIQtyTable = () => {
    ...
    const columns = [
        ...
        { label: "DO Date", index: 3 },
        { label: "Shipment Date", index: 4 },
        { label: "Shipment Driver", index: 5 },
        { label: "AWB No", index: 6 },
        { label: "Est. POD Date", index: 7 },
        { label: "POD Date", index: 8 },
        { label: "Cust GR No", index: 9 }
    ];
    ...
    useEffect(() => {
        axios.get(`${__API_URL__}/outstanding_si_qty/master`)
            .then(res => setOutstandingSIQtyData(res.data))
            .catch(err => console.error(err));
    }, []); 
    useEffect(() => {
        if (!outstandingSIQtyData || outstandingSIQtyData.length === 0) return;
        ...
        tableInstance.current = $(tableRef.current).DataTable({
            ...
            columns: [
                ...
                { title: "Shipment Driver", data: "shipment_driver" },
                { title: "AWB No", data: "awb_no" },
                { title: "Est. POD Date", data: "est_pod_date" },
